...
int sk_setbufs(int sk, uint32_t *bufs)
{
	...
	if (restore_opt(sk, SOL_SOCKET, SO_SNDBUFFORCE, &sndbuf) ||
	    restore_opt(sk, SOL_SOCKET, SO_RCVBUFFORCE, &rcvbuf)) {
		if (opts.unprivileged) {
			pr_info("Unable to set SO_SNDBUFFORCE/SO_RCVBUFFORCE, falling back to SO_SNDBUF/SO_RCVBUF\n");
			if (restore_opt(sk, SOL_SOCKET, SO_SNDBUF, &sndbuf) ||
			    restore_opt(sk, SOL_SOCKET, SO_RCVBUF, &rcvbuf)) {
				return -1;
			}
		}
		else {
