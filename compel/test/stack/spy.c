...
static int page_writable(struct parasite_ctl *ctl, int pid, void *page)
{
	...
	int written = snprintf(victim_maps_path, sizeof(victim_maps_path), "/proc/%d/maps", pid);
				...
				size_t *saved_data_size)
{
	...
	if (saved_size > 0) {
		void *sd = read_proc_mem(pid, stack, saved_size);
		if (sd == NULL)
			return -1;
		*saved_data = sd;
	} else
...
{
	if (saved_data) {
		void *current_data = read_proc_mem(pid, stack, saved_data_size);
		return memcmp(saved_data, current_data, saved_data_size);
	}
...
static int do_infection(int pid)
{
	...
	*arg = 404;
	...
	if (check_saved_data(ctl, pid, stack, saved_data, saved_data_size))
